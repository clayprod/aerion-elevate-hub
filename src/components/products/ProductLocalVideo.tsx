...
interface ProductLocalVideoProps {
  videos: LocalVideo[];
  title: string;
}

export const ProductLocalVideo: React.FC<ProductLocalVideoProps> = ({
  videos,
  title
}) => {
        ...
        {videos.map((video, index) => (
          <Card key={index} className="overflow-hidden hover:shadow-lg transition-all duration-300 group cursor-pointer">
            <div 
              ...
              onClick={() => openVideo(video.videoPath)}
            >
              {video.thumbnail ? (
                <img
                  src={video.thumbnail}
                ...
                <div className="w-full h-full bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center">
                  <Play className="w-16 h-16 text-blue-medium opacity-50" />
                </div>
              )}
