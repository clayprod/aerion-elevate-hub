-- Fix: Resolver ambiguidade de table_name renomeando variáveis para evitar conflitos
-- Esta migration corrige definitivamente o problema de "table_name is ambiguous"

-- Fix create_site_backup function - renomear variável para evitar ambiguidade
...
DECLARE
  ...
  tbl_name TEXT;  -- Renomeado de table_name para tbl_name
  table_data JSONB;
...
BEGIN
  ...
  -- Backup each table
  FOREACH tbl_name IN ARRAY tables_to_backup
  LOOP
    -- Check if table exists (usando tbl_name para evitar ambiguidade)
    IF EXISTS (
      ...
      EXECUTE format('SELECT json_agg(row_to_json(tbl)) FROM (SELECT * FROM public.%I) tbl', tbl_name)
      INTO table_data;
...
-- Fix restore_site_backup function - renomear variável para evitar ambiguidade
...
BEGIN
  ...
  -- Restore each table
  FOREACH tbl_name IN ARRAY backup_record.tables_backed_up
  LOOP
    -- Check if table exists (usando tbl_name para evitar ambiguidade)
    IF EXISTS (
      SELECT 1 FROM information_schema.tables t
      ...
      table_data := backup_record.backup_data->tbl_name;
